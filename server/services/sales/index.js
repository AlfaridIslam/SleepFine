...
const app = express();
const port = config.services.sales.port || 3001;
...
const setupMiddleware = () => {
  ...
  logger.info('Sales Service: Middleware setup completed');
};
...
const setupRoutes = () => {
  ...
  app.get('/', (req, res) => {
    res.status(200).json({
      success: true,
      message: 'SleepFine Sales Service API',
      version: '1.0.0',
      documentation: '/api/v1/sales/health',
      timestamp: new Date().toISOString()
    });
  });
  ...
  logger.info('Sales Service: Routes setup completed');
};
...
const initializeDatabase = async () => {
  try {
    await connectDB();
    logger.info('Sales Service: Database connected successfully');
  } catch (error) {
    logger.error('Sales Service: Database connection failed:', error);
    throw error;
  }
};
...
const startServer = () => {
  const server = app.listen(port, () => {
    ...
    logger.info(`📝 API Documentation: http://localhost:${port}/api/v1/sales`);
  });
  ...
  const gracefulShutdown = (signal) => {
    ...
    server.close((err) => {
      ...
      logger.info('Sales Service: Server closed successfully');
      process.exit(0);
    });
...
const initializeSalesService = async () => {
  try {
    ...
    logger.info('✅ Sales Service initialized successfully');
    return { app, server };
...
// Start the service
const salesService = await initializeSalesService();
